$(document).ready(function(){
	$.get("/ProductList",function(data){
		    ...
		    $('#purchaseby').append($('<option>').text('-Product-'))
	   $.each(data,function(i,item){
		 $('#purchaseby').append($('<option>').text(item.productname).val(item.productid))  
	   })	  
	   })
	   
	   $('#btn').click(function(){ 
		   
		   $.get("/ReportPurchaseByProducts",{ajax:true,productid:$('#purchaseby').val()},function(data){
